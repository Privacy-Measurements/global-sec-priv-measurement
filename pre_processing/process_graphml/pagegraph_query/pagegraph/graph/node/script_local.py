...
class ScriptLocalNode(ScriptNode, Reportable):
    ...
    def creator_node(self) -> Optional[Union[ActorNode, ParentDOMElementNode]]:
        execute_edge = self.execute_edge()
        if not execute_edge:
         #   print(f"[WARN] ScriptLocalNode {self.pg_id()} has no execute edge; returning None as creator_node.")
            return None
    ...
    def to_report(self, include_source: bool = False) -> Optional[ScriptReport]:
        executor_report: Union[ScriptReport, DOMElementReport, None] = None
        executor_node = self.creator_node()

        if executor_node is None:
         #   print(f"[WARN] ScriptLocalNode {self.pg_id()} has no executor_node; skipping report.")
            return None  
...
#        assert matching_request_chain
        if not matching_request_chain:
          #  print(f"[WARN] ScriptLocalNode {self.pg_id()} has no matching request chain; returning None as URL.")
            return None
