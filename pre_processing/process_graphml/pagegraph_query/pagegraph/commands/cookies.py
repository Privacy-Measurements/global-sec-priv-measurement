...
class Command(pagegraph.commands.Base):
    ...
    def get_key_of_caller(self, edge_report):
        ...
        if edge_report['incoming node']['type'] == 'cookie jar':
            return 'outgoing node'

        elif edge_report['outgoing node']['type'] == 'cookie jar':
    ...
    def execute(self) -> pagegraph.commands.Result:
        ...
        cookie_jar = pg.cookie_nodes()

        if len(cookie_jar) > 1:
            throw('BECAREFUL MORE THAN ONE COOKIEJAR:', len(cookie_jar), self.input_path)
            return

        if len(cookie_jar) == 0:
            return {}

        cookie_jar = cookie_jar[0]
        outgoing_edges = cookie_jar.outgoing_edges()
        incoming_edges = cookie_jar.incoming_edges()
        ...
        for outgoing_edge in outgoing_edges + incoming_edges:
            ...
            report['cookie_key'] = edge_data['key']
            report['cookie_value'] = edge_data.get('value', 0)
            report['caller'] = {}
