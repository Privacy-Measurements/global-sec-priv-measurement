...
import pagegraph.commands.cookies
...
def get_command(args: argparse.Namespace) -> pagegraph.commands.Base:
    match args.command_name:
        ...
        case "cookies":
            return pagegraph.commands.cookies.Command(
                args.input, args.frame, args.id, args.debug)
        case "unknown":
...
COOKIES_PARSER = SUBPARSERS.add_parser(
    "cookies",
    help="Print information about cookies created/edited/accessed during page execution.")
...
COOKIES_PARSER.add_argument(
    "-f", "--frame",
    default=None,
    help="Only include cookies created/accessed/edition from a script in a particular frame "
         "context (as described by PageGraph node ids, in the format 'n##'). ")
COOKIES_PARSER.add_argument(
    "-i", "--id",
    default=None,
    help="If provided, only print information about cookie nodes with the given "
         "ID (as described by PageGraph node ids, in the format 'n##').")
COOKIES_PARSER.add_argument(
    "-s", "--source",
    default=True,
    action="store_true",
    help="If included, also include cookies source in each report.")
COOKIES_PARSER.add_argument(
    "-o", "--omit-executors",
    default=False,
    action="store_true",
...
COOKIES_PARSER.set_defaults(command_name="cookies")
